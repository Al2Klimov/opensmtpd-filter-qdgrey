...
mod tests {
    ...
    const BINARY: &str = "../opensmtpd-filter-qdgrey";
    ...
    impl FilterProcess {
        fn start() -> Self {
            ...
            FilterProcess { child, stdin, lines_rx: rx }
        }
    ...
    fn test_handshake() {
        ...
        assert_eq!(filter.read_line(), "register|report|smtp-in|link-disconnect");
    ...
    fn test_still_greylisted_retry_within_window() {
        ...
        assert!(r1.contains("reject|450 Greylisted"), "Expected greylisted on first attempt, got: {}", r1);
    ...
    fn test_whitelist_ttl_extended() {
        ...
        filter_rcpt_to(&mut filter, "sess2", "req2", "recipient@example.com");
        let r2 = filter.read_line();
        assert!(r2.contains("proceed"), "Expected proceed after grey expiry, got: {}", r2);
